...
import { ArrowLeft, Edit3, Download, Clock, CheckCircle, AlertCircle } from 'lucide-react';
...
const ContratDetail = () => {
    ...
    const loadDetails = async () => {
        try {
            ...
            setHistory(hist.filter(v => !v.is_active)); // History only contains inactive versions usually, or all? Api returns all. Let's filter visually.
            // Actually API likely returns all including active.
            // Let's store all in history for the list, and separate active.
            setHistory(hist);
        } catch (error) {
            console.error(error);
        } finally {
